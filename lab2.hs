...
toDigits a
  | a < 0       = error "should be > 0"
  | a >= 10     = toDigits (div a 10) ++ [mod a 10]
  | otherwise   = [a]
...
doubleSecond xs = [(xs !! n) * toInteger(1 + n `mod` 2) | n <- [0..n-1]]
  where n = length xs
...
isValid a = sumDigits (doubleSecond (toDigitsRev a)) `mod` 10 == 0
